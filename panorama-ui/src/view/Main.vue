      ...
      <div slot="header">
        <div class="card-title">
          ...
          <div class="card-property">
            <span ref="property" v-show="false"></span>
          </div>
        </div>
      </div>
...
export default {
      ...
      pictureMap: [
        {
          ...
          name: "home3", //地址标识，当创建 map 的时候自动创建 map 的对应图片对象，name 属性与 map 对象的 label 命名一致，地点的详细项命名规则为地点 + “_” + 地点详细项，如 home1_printer
          ...
          buttons: [
            ...
            },
            {
              ...
              buttonType: "jump", // 按钮的类型：jump（跳转到另外一个场景），report，data等，暂时没想好
              paras: "home1", // 传给按钮动作的参数，因为考虑到可能要非 IT 人员配置，这里直接用 “,” 分隔的字符串
            },
          ],
        },
        {
  ...
  methods: {
    ...
    handleSenceChange(data, node) {
      var targetSence = this.pictureMap.find(
    ...
    modelling() {
      ...
      first.buttons.forEach((element) => {
        var button = element;
        var canvasText = this.getcanvers(
          button.buttonName,
    ...
    getcanvers(text, buttonLocation) {
      //生成一个 canvas 图案
